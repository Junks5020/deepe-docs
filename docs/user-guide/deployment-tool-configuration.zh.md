...
**部署工具配置（Deployment Tool Configuration）** 模块允许管理员在 DeepExtension 中设置和管理用于部署 **完整模型（Complete Models）** 的运行环境。该配置对于与本地部署工具（如 **Ollama**）的无缝集成至关重要，未来还计划支持更多工具，如 **LM Studio**。
